...
class GameTimerList
{
    ...
    {
        m_count -= sub;
        //DLogger.Assert(m_count >= 0);
    }

    public void AddSorted(GameTimer node, float triggerTime)
    {
        //DLogger.Assert(node.m_prev == null);
        //DLogger.Assert(node.m_next == null);
...
public class GameTimerMgr : BehaviourSingleton<GameTimerMgr>
{
    ...
    public override void Awake()
    {
        DoCreateLoopTimer("TimerDebug", 10f, () =>
        {
            ...
            var totalCount = m_frameUpdateList.m_count + m_frameLateUpdateList.m_count +
                             +m_frameOnceUpdateList.m_count + m_runningList.m_count + m_unscaleRunningList.m_count;

            int maxTriggerCount = 2000;//Platform.IsPcOrEditorPlatform() ? 1000 : 2000;
            if (totalCount > maxTriggerCount)
            {
                PrintDebug();
                //DBugly.ReportException("Timer is too manay", "GameTimer is overflower", Environment.StackTrace);
                //DLogger.EditorFatal("Timer is overflow: {0}", totalCount);
            }
        });
    }
    ...
    {
        Debug.LogFormat("timer count: {0} updaet count:{1}, late update count:{2}, once update count:{3}",
        ...
        Debug.LogFormat("------------------LateUpdate---------------------");
        PrintTimerListStatic(m_frameLateUpdateList);

        Debug.LogFormat("------------------Running---------------------");
        PrintTimerListStatic(m_runningList);

        Debug.LogFormat("------------------Unscale Running---------------------");
    ...
    private void ProcessDestroyTimer(GameTimer timer)
    {
        ...
        if (!timer.m_inExeQueue)
        {
            ...
            {
                //DLogger.Assert(type == GameTimerType.TimerFrameLateUpdate);
                m_frameLateUpdateList.Remove(timer);
            }
        }
        else
        {
            Debug.Log("Free when in exuete queue");
        }
    }
    ...
    {
        Profiler.BeginSample("UpdateTickTimer");
        UpdateTickTimer(false);
        Profiler.EndSample();

        Profiler.BeginSample("UpdateUnscaleTickTimer");
        UpdateTickTimer(true);
        Profiler.EndSample();

        Profiler.BeginSample("UpdateFrameTimer");
        UpdateFrameTimer(m_frameUpdateList);
        Profiler.EndSample();

        Profiler.BeginSample("UpdateOnceFrameTimer");
    ...
    private void UpdateTickTimer(bool isUnscaled)
    {
        ...
        if (head != null)
        {
            ...
            {
                //DLogger.Assert(!node.m_destroyed);
            ...
            if (node != null)
            {
                ...
                //DLogger.Assert(runningList.m_count >= 0);
            ...
            while (node != null)
            {
                ...
                {
                    Debug.LogFormat("destroy timer: {0}", node.m_sourceName);
                }
    ...
    private void PrintTimerListStatic(GameTimerList list)
    {
        ...
        while (itr.MoveNext())
        {
            Debug.LogFormat("{0}:{1}", itr.Current.Key, itr.Current.Value);
        }
    }
    ...
    private void UpdateFrameTimer(GameTimerList list)
    {
        if (list == null)
        {
            Debug.LogFormat("Error: GameTimerList list in null !".ToColor("FF0000"));
            return;
        }
        ...
        while (node != null)
        {
            //DLogger.Assert(!node.m_destroyed, node.m_sourceName);

            node.m_inExeQueue = true;

            Profiler.BeginSample(node.m_sourceName);

            if (node.m_callAction != null)
            {
                node.m_callAction();
            }
            ...
            if (node.m_destroyed)
            {
                Debug.Log("timer is destroy when loop, free it");
                list.Remove(node);
            }
